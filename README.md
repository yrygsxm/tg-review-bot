...
- 用户小程序支持上传 1 张配图，图片必须小于 1MB，支持 JPG、PNG、WebP。
- 用户也可以私聊机器人直接发送或转发文字、图片、视频、文件；机器人会询问是否确认投稿。
- 投稿进入管理员审核聊天，并保留 `发送 / 驳回` 消息按钮。
- 管理员可在小程序里查看待审核列表和投稿配图，编辑投稿文案后点击发送。
- 管理员可在小程序里查看历史记录，确认投稿文字、发送成功或驳回状态。
- 管理员可在小程序里维护固定驳回理由、正文关键词黑名单和用户精准黑名单。
- 投稿命中黑名单关键词时，系统会自动驳回。
- 用户精准黑名单支持按 Telegram 用户 ID、@用户名、昵称精确匹配。
- 图片文件 ID 会在投稿创建 7 天后由 Worker 定时任务从 D1 清理，只保留文字和处理记录。
- 默认限制同一用户 1 小时内只能投稿 1 次，管理员可用命令调整。
...
3. 打开 `/app`，普通用户应看到投稿页和可选配图上传。
4. 管理员打开 `/app`，应看到 `审核 / 历史 / 规则` 三个页签，不再显示投稿页。
5. 添加一个黑名单关键词，提交包含该关键词的投稿，应自动驳回。
6. 提交正常投稿，审核群应收到审核消息。
7. 在小程序审核页编辑文案并点击发送，频道应收到最终消息。
8. 私聊机器人直接发送或转发内容，点击确认投稿后，管理员小程序应出现待审核记录。
...
- `/list_blacklist`
- `/del_blacklist <ID>`
- `/add_user_blacklist <用户ID|@用户名|昵称>`
- `/list_user_blacklist`
- `/del_user_blacklist <ID>`
- `/set_rate_limit <分钟>`
- `/rate_limit`
- `/add_admin <用户ID>`
- `/del_admin <用户ID>`
...
- 小程序支持文字和 1 张小于 1MB 的配图。
- 投稿配图的 D1 文件引用保留 7 天；之后历史记录仍保留文字、作者和处理状态，但不再显示图片。
- 视频、文件投稿继续走私聊机器人发送。
